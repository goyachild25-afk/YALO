...
- ~~Limpieza de políticas RLS redundantes en `bookings`~~ — hecho 2026-07-16: 13 políticas (6 SELECT + 6 UPDATE + 1 INSERT) reducidas a 6, sin solapes. De paso se encontraron y cerraron 2 políticas UPDATE sin `WITH CHECK` (dejaban en teoría reescribir cualquier columna de la propia reserva sin restricción) — verificado contra cada `.update()` real del código, ningún flujo dependía de eso. Otras tablas con acumulación menor (`profiles`, `provider_profiles`, `reviews`, `provider_services`) quedan pendientes de la misma revisión, sin urgencia.
